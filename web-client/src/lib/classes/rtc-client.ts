...
export class RTCClient {
	...
	public async createOffer(
		...
		if (!this.connection) {
			console.warn(`[RTCClient(${this.userId})]: cannot create offer cause no connection`);
			return null;
		}
	...
	public async createAnswer(
		...
		if (!this.connection) {
			console.warn(`[RTCClient(${this.userId})]: cannot create answer cause no connection`);
			return null;
		}
	...
	public async applyAnswer(sdp: string): Promise<void> {
		if (!this.connection) {
			console.warn(`[RTCClient(${this.userId})]: cannot apply answer cause no connection`);
			return;
		}
	...
	public async applyCandidate(candidate: string): Promise<void> {
		if (!this.connection) {
			console.warn(`[RTCClient(${this.userId})]: cannot apply candidate cause no connection`);
			return;
		}
	...
	public async setRemoteDescription(type: RTCSdpType, sdp: string): Promise<void> {
		if (!this.connection) {
			console.warn(`[RTCClient(${this.userId})]: cannot set remote description no connection`);
			return;
		}

		await this.connection.setRemoteDescription(new RTCSessionDescription({ type: type, sdp }));
	}
	...
	private handleOnTrackEvent(event: RTCTrackEvent): void {
		...
		addRemoteStream(this.userId, remoteStream);
	}
